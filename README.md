...
- **Configuration-First**: Manage models, API keys, and cloud/live settings via `config.toml`.
...
graph TD
    ...
    B --> C{Temporal Split}
    C --> D[Model Training]
    D -- Saved Model & Preprocessor --> F[Predict Live Server]
    F --> G[Web UI (Frontend)]
    C -- Validation/Test Set --> E[Model Evaluation]
    E --> H[Risk Management Backtest]
...
To integrate advanced sentiment features from cloud LLMs, edit `config.toml`:
...
# provider = "google" # Options: openai, anthropic, google (requires respective client libs)
...
***
*Replace `https://github.com/your-username/finana.git` with the actual URL of your GitHub repository.*